...
// Add the balancing board with a cylinder base and a board with an edge for the
// ball to rest on.
function addBalancingBoard(renderInfo, physicsInfo, ammoHelper, position) {
  // The board cylinder base
  const cylinder = new Cylinder(0.5, 5, materials.wood);
  ...
  const board = new Box(20, 0.5, 3, materials.wood);
...
function addBall(renderInfo, physicsInfo, ammoHelper, position, mass, name) {
  ...
  rigidBody.setCollisionMask =
    physicsInfo.collisionGroup.board ||
    physicsInfo.collisionGroup.aquarium ||
